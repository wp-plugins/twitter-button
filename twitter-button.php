...
function twitter_button() {
	...
	if (get_settings('tb-tweet-text') == "1") {
		$tb_tweet_text = urlencode($post->post_title);
	}
	else if (get_settings('tb-tweet-text') == "2") {
		$tb_tweet_text = urlencode($post->post_title .' - '. get_bloginfo('name'));
	}
	else if (get_settings('tb-tweet-text') == "3") {
		$tb_tweet_text = urlencode(get_bloginfo('name'));
	}
	else {
		$tb_tweet_text = urlencode(get_settings('tb-url-custom'));
	}
	...
	$tweet_button = '<a href="http://twitter.com/share" class="twitter-share-button" data-count="'. $tb_button .'" data-text="'. $tb_tweet_text .'" data-url="'. $tb_url .'" '.$tb_user.$tb_lang.'>Tweet</a><script type="text/javascript" src="http://platform.twitter.com/widgets.js"></script>';
